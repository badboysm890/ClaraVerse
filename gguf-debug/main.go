...
func investigateGGUF(modelPath string) error {
	...
	for i := uint64(0); i < metadataCount && i < 20; i++ { // Limit to first 20 entries
	...
	fmt.Printf("\n📊 SUMMARY:\n")
	fmt.Printf("  Successful entries: %d/%d\n", successCount, minInt(20, int(metadataCount)))
	...
	if successCount == 0 {
		...
		return investigateRawBytes(file, modelPath)
	}
...
func investigateRawBytes(file *os.File, modelPath string) error {
