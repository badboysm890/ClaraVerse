...
import angelaSidebar from './angela_Components/angelaSidebar';
import angelaAssistantInput from './angela_Components/angela_assistant_input';
import angelaChatWindow from './angela_Components/angela_assistant_chat_window';
import { AdvancedOptions } from './angela_Components/angela_assistant_input';
import Sidebar from './Sidebar';
import { db } from '../db';
import { angelaDB } from '../db/angelaDatabase';

// Import angela types and API service
import { 
  angelaMessage, 
...
import { angelaMCPService } from '../services/angelaMCPService';
import { angelaMemoryService } from '../services/angelaMemoryService';
...
import { angelaBackgroundService } from '../services/angelaBackgroundService';

// Import clear data utility
import '../utils/clearangelaData';
import { copyToClipboard } from '../utils/clipboard';

// Import the new professional status panel
import AutonomousAgentStatusPanel from './angela_Components/AutonomousAgentStatusPanel';
import useAutonomousAgentStatus from '../hooks/useAutonomousAgentStatus';

// Import TTS service
import { angelaTTSService } from '../services/angelaTTSService';

  // Import artifact detection service
import ArtifactDetectionService, { DetectionContext } from '../services/artifactDetectionService';

// Import angelaSweetMemory for automatic memory extraction (legacy)
import angelaSweetMemory, { angelaSweetMemoryAPI } from './angelaSweetMemory';

// Import angelaMemoryToast for learning notifications
import angelaMemoryToast from './angela_Components/angelaMemoryToast';
import { angelaMemoryToastService, type MemoryToastState } from '../services/angelaMemoryToastService';

// Import the new memory integration service
import { angelaMemoryIntegration } from '../services/angelaMemoryIntegration';

// angela Memory Dashboard
import angelaBrainDashboard from './angela_Components/angelaBrainDashboard';
...
interface angelaAssistantProps {
 ...
 */
const getDefaultSystemPrompt = (provider: angelaProvider, artifactConfig?: any, userInfo?: { name?: string; email?: string; timezone?: string }): string => {
  ...
  const enhanceSystemPromptWithMemory = async (basePrompt: string): Promise<string> => {
    try {
      const memoryProfile = await angelaSweetMemoryAPI.getCurrentUserProfile();
      if (!memoryProfile) {
        return basePrompt;
      }
...
`;
  ...
  switch (provider?.type) {
    case 'ollama':
      return `${userContext}You are angela, a helpful AI assistant powered by ${providerName}. You are knowledgeable, friendly, and provide accurate information. You can help with various tasks including analysis, coding, writing, and general questions. When using tools, be thorough and explain your actions clearly.${artifactGuidance} ${toolsGuidance}`;
      
    case 'openai':
      return `${userContext}You are angela, an intelligent AI assistant powered by OpenAI. You are helpful, harmless, and honest. You excel at reasoning, analysis, creative tasks, and problem-solving. Always strive to provide accurate, well-structured responses and use available tools effectively when needed.${artifactGuidance} ${toolsGuidance}`;
      
    case 'openrouter':
      return `${userContext}You are angela, a versatile AI assistant with access to various models through OpenRouter. You adapt your communication style based on the task at hand and leverage the strengths of different AI models. Be helpful, accurate, and efficient in your responses.${artifactGuidance} ${toolsGuidance}`;
      
    case 'angelas-pocket':
      return `${userContext}# angela - Privacy-First AI 🎯

You're angela, a tech-savvy friend on user's device. Be real, helpful, chill.
...
**Remember:** Knowledge friend who wants to help. When limited, suggest agent mode for full capabilities..${artifactGuidance} ${toolsGuidance}`;
    ...
    default:
      return `${userContext}You are angela, a helpful AI assistant. You are knowledgeable, friendly, and provide accurate information. You can help with various tasks including analysis, coding, writing, and general questions. Always be helpful and respectful in your interactions.${artifactGuidance} ${toolsGuidance}`;
  }
};

// Add a hook to detect if angela is currently visible
const useIsVisible = () => {
  const [isVisible, setIsVisible] = useState(true);
  
  useEffect(() => {
    const checkVisibility = () => {
      // Check if the angela container is visible
      const angelaContainer = document.querySelector('[data-angela-container]');
      if (angelaContainer) {
        const isCurrentlyVisible = !angelaContainer.classList.contains('hidden');
    ...
    const angelaContainer = document.querySelector('[data-angela-container]');
    if (angelaContainer) {
      observer.observe(angelaContainer, { 
        attributes: true, 
...
const angelaAssistant: React.FC<angelaAssistantProps> = ({ onPageChange }) => {
  // Check if angela is currently visible (for background operation)
  const isVisible = useIsVisible();
  
  // User and session state
  const [userName, setUserName] = useState<string>('');
  const [userInfo, setUserInfo] = useState<{ name?: string; email?: string; timezone?: string } | null>(null);
  const [currentSession, setCurrentSession] = useState<angelaChatSession | null>(null);
  const [messages, setMessages] = useState<angelaMessage[]>([]);
  ...
  const [sessions, setSessions] = useState<angelaChatSession[]>([]);
  ...
  const [sessionConfig, setSessionConfig] = useState<angelaSessionConfig>({
    aiConfig: {
      models: {
        text: '',
        vision: '',
  ...
  const checkProviderHealthCached = useCallback(async (provider: angelaProvider): Promise<boolean> => {
    const now = Date.now();
  ...
  const checkServiceStartupStatus = useCallback(async (): Promise<{
    ...
    try {
      // Check angela's Pocket service status - check regardless of current provider setting
  ...
  const refreshProvidersAndServices = useCallback(async (force: boolean = false) => {
    ...
    try {
      ...
      try {
        ...
        await angelaMCPService.refresh();
      ...
      const refreshedProviders = await angelaApiService.getProviders();
      ...
      if (currentProviderId) {
        ...
        if (currentProvider) {
          angelaApiService.updateProvider(currentProvider);
      ...
      if (sessionConfig.aiConfig?.provider) {
        ...
        if (currentProvider) {
          try {
            const isHealthy = await angelaApiService.testProvider(currentProvider);
            if (!isHealthy) {
  ...
  // Track angela's visibility state for background service
  useEffect(() => {
    angelaBackgroundService.setBackgroundMode(!isVisible);
  }, [isVisible]);

  // Auto-refresh when angela becomes visible again
  useEffect(() => {
    if (isVisible && !isLoadingProviders) {
      // Trigger refresh when angela becomes visible
      console.log('👁️ angela became visible - checking for updates...');
  ...
  useEffect(() => {
    const loadInitialSessions = async () => {
      ...
      try {
        ...
        const recentSessions = await angelaDB.getRecentangelaSessionsLight(20); // Load only 20 initially
        ...
        if (!currentSession && recentSessions.length > 0) {
          ...
          const sessionWithMessages = await angelaDB.getangelaSession(mostRecent.id);
  ...
  const loadMoreSessions = useCallback(async () => {
    ...
    try {
      const moreSessions = await angelaDB.getRecentangelaSessionsLight(20, sessionPage * 20);
  ...
  useEffect(() => {
    const loadProvidersAndModels = async () => {
      ...
      try {
        ...
        angelaApiService.clearBlacklistedTools();
        
        // Initialize MCP service
        try {
          await angelaMCPService.initialize();
        ...
        for (const provider of loadedProviders) {
          try {
            // Special handling for angela's Pocket provider - wait for service to be ready
            if (provider.type === 'angelas-pocket' && window.llamaSwap) {
              const status = await window.llamaSwap.getStatus?.();
              if (status?.isStarting) {
                console.log(`⏳ angela's Core is starting, skipping model loading for now...`);
        ...
        if (primaryProvider) {
          // AUTO-START angela'S POCKET IF IT'S THE PRIMARY PROVIDER
          if (primaryProvider.type === 'angelas-pocket' && window.llamaSwap) {
            try {
              console.log("🚀 Checking angela's Core status on startup...");
              const status = await window.llamaSwap.getStatus?.();
              if (!status?.isRunning) {
                console.log("🔄 angela's Core is not running, starting automatically...");
                addInfoNotification(
                  "Starting angela's Core...",
                  'angela is starting up her local AI service for you. This may take a moment.',
                  6000
                );
                ...
                if (!result.success) {
                  addErrorNotification(
                    "Failed to Start angela's Core",
                    result.error || 'Could not start the local AI service. Please check your installation.',
                    10000
                  );
                  console.error("❌ Failed to start angela's Core:", result.error);
                } else {
                  console.log("✅ angela's Core started successfully");
                  addInfoNotification(
                    "angela's Core Ready",
                ...
                console.log("✅ angela's Core is already running");
                addInfoNotification(
                  "angela's Core Online",
                  'Your local AI service is ready and waiting for your messages.',
                  3000
                );
              }
            } catch (err) {
              console.error("⚠️ Error checking/starting angela's Core:", err);
              addErrorNotification(
                "angela's Core Startup Error",
          ...
          angelaApiService.updateProvider(primaryProvider);
  ...
  useEffect(() => {
    const initializeTTS = async () => {
      try {
        ...
        const isHealthy = await angelaTTSService.forceHealthCheck();
...
Now tell me what is the result "`;
      ...
      const refinementResponse = await angelaApiService.sendChatMessage(
        refinementPrompt,
        streamingConfig,
  ...
  // Create new session
  const createNewSession = useCallback(async (): Promise<angelaChatSession> => {
    try {
      const session = await angelaDB.createangelaSession('New Chat');
  ...
  const handleSendMessage = useCallback(async (content: string, attachments?: angelaFileAttachment[]) => {
    ...
    const userMessage: angelaMessage = {
      id: generateId(),
      role: 'user',
    ...
    if (!isVisible) {
      angelaBackgroundService.incrementBackgroundActivity();
    }

    // Save user message to database (with display content only)
    try {
      await angelaDB.addangelaMessage(currentSession.id, userMessage);
    ...
    const streamingMessage: angelaMessage = {
    ...
    try {
      ...
      const conversationHistory: angelaMessage[] = [];
      ...
      let baseSystemPrompt = enforcedConfig.systemPrompt || 
                          (currentProvider ? getDefaultSystemPrompt(currentProvider, enforcedConfig.artifacts, userInfo || undefined) : 'You are angela, a helpful AI assistant.');
      ...
      if (enforcedConfig.features?.enableMemory !== false) { // Default to true if not specified
        try {
          systemPrompt = await angelaMemoryIntegration.enhanceSystemPromptWithMemory(baseSystemPrompt, userInfo || undefined);
      ...
      const aiMessage = await angelaApiService.sendChatMessage(
      ...
      if (aiMessage.content && aiMessage.content.length > 50) {
        ...
        if (autoDetectEnabled) {
          const detectionContext: DetectionContext = {
            userMessage: content,
            conversationHistory: conversationHistory.map((msg: angelaMessage) => msg.content),
      ...
      try {
        await angelaDB.addangelaMessage(currentSession.id, finalMessage);
          ...
          if (enforcedConfig.features?.enableMemory !== false && content.length <= 2000 && content.trim().length > 10) {
            ...
            setTimeout(async () => {
              try {
                ...
                const success = await angelaMemoryIntegration.processConversationMemory(
                ...
                if (success) {
                  ...
                  try {
                    const updatedProfile = await angelaMemoryIntegration.getCurrentUserProfile();
                    if (updatedProfile) {
                      angelaMemoryToastService.showMemoryToast(updatedProfile);
                    }
                  } catch (toastError) {
        ...
        if (!isVisible) {
          // More prominent notification when angela is in background
          // Use display content for notifications (without voice prefix)
          addBackgroundCompletionNotification(
            'angela Response Ready',
            `angela has finished responding to: "${displayContent.slice(0, 40)}${displayContent.length > 40 ? '...' : ''}"`
          );
          // Track background notification creation
          angelaBackgroundService.onBackgroundNotificationCreated();
        } else {
          // Standard notification when angela is visible
          addCompletionNotification(
            'Chat Response Complete',
            isVoiceMessage ? 'angela has finished responding to your voice message.' : 'angela has finished responding to your message.',
            4000
          );
        }
      } catch (error) {
      ...
      if (currentSession.title === 'New Chat') {
        ...
        try {
          await angelaDB.updateangelaSession(currentSession.id, { title: newTitle });
          setSessions(prev => prev.map(s => 
      ...
      if (isAbortError) {
        ...
        try {
          ...
          setMessages(prev => {
            ...
            if (currentMessage && currentSession) {
              ...
              angelaDB.addangelaMessage(currentSession.id, abortedMessage).catch(dbError => {
        ...
        if (isVisionError || (hasImages && error instanceof Error && error.message.includes('server'))) {
          ...
          const errorMessage: angelaMessage = {
            id: streamingMessageId,
            role: 'assistant',
...
• Start a new chat if the conversation is too long`,
        ...
        try {
          ...
          setMessages(prev => {
            const currentMessage = prev.find(msg => msg.id === streamingMessageId);
            if (currentMessage && currentSession) {
              angelaDB.addangelaMessage(currentSession.id, currentMessage).catch(dbError => {
      ...
      if (!isVisible) {
        angelaBackgroundService.decrementBackgroundActivity();
      }
  ...
  const handleSelectSession = useCallback(async (sessionId: string) => {
    ...
    try {
      const session = await angelaDB.getangelaSession(sessionId);
      if (session) {
  ...
  const handleSessionAction = useCallback(async (sessionId: string, action: 'star' | 'archive' | 'delete') => {
    try {
      if (action === 'delete') {
        ...
        await angelaDB.deleteangelaSession(sessionId);
        ...
        if (currentSession?.id === sessionId) {
          ...
          if (remainingSessions.length > 0) {
            ...
            const nextSession = await angelaDB.getangelaSession(remainingSessions[0].id);
            if (nextSession) {
        ...
        await angelaDB.updateangelaSession(sessionId, updates);
        setSessions(prev => prev.map(s => 
  ...
  const handleProviderChange = useCallback(async (providerId: string) => {
    try {
      ...
      if (provider.type === 'angelas-pocket' && window.llamaSwap) {
        try {
          console.log("🚀 Switching to angela's Core - checking status...");
          // Check if running
          const status = await window.llamaSwap.getStatus?.();
          if (!status?.isRunning) {
            console.log("🔄 angela's Core is not running, starting for provider switch...");
            addInfoNotification(
              "Starting angela's Core...",
              'angela is starting up her local AI service. Please wait a moment.',
              6000
            );
            const result = await window.llamaSwap.start();
            if (!result.success) {
              ...
              console.error("❌ Failed to start angela's Core for provider switch:", result.error);
              setIsLoadingProviders(false);
              return;
            }
            console.log("✅ angela's Core started successfully for provider switch");
            addInfoNotification(
              "angela's Core Ready",
            ...
            console.log("✅ angela's Core is already running for provider switch");
          }
        } catch (err) {
          console.error("⚠️ Error starting angela's Core for provider switch:", err);
          addErrorNotification(
            "angela's Core Startup Error",
            err instanceof Error ? err.message : 'Could not communicate with the local AI service.',
            8000
      ...
      angelaApiService.updateProvider(provider);
      
      // STEP 3: Load models ONLY from the selected provider
      const newModels = await angelaApiService.getModels(providerId);
  ...
  const handleRetryMessage = useCallback(async (messageId: string) => {
    ...
    try {
      ...
      if (currentSession) {
        try {
          await angelaDB.deleteMessage(currentSession.id, userMessage.id);
          await angelaDB.deleteMessage(currentSession.id, assistantMessage.id);
  ...
  const handleEditMessage = useCallback(async (messageId: string, newContent: string) => {
    ...
    if (currentSession) {
      try {
        await angelaDB.updateangelaMessage(currentSession.id, messageId, {
          content: newContent,
          timestamp: new Date()
        });
  ...
  const handlePreloadModel = useCallback(async () => {
    ...
    if (sessionConfig.aiConfig.provider === 'angelas-pocket') {
      try {
        const status = await window.llamaSwap?.getStatus();
        if (!status?.isRunning) {
          console.log('🚀 Starting local server...');
          await angelaApiService.preloadModel(sessionConfig.aiConfig, messages);
        }
  ...
  const handleConfigChange = useCallback((newConfig: Partial<angelaSessionConfig>) => {
    setSessionConfig(prev => {
  ...
  useEffect(() => {
    ...
    (window as any).debugangelaProviders = () => {
    ...
    (window as any).debugMCP = async () => {
      ...
      console.log('MCP Service Ready:', angelaMCPService.isReady());
      console.log('Available Servers:', angelaMCPService.getRunningServers());
      console.log('Available Tools:', angelaMCPService.getAvailableTools());
    ...
    (window as any).setupTestMCP = async () => {
      ...
      const success = await angelaMCPService.setupBundledPythonMCPServer();
      if (success) {
        console.log('✅ Bundled Python MCP server setup complete');
        await angelaMCPService.refresh();
        console.log('📊 Updated MCP status:', {
          servers: angelaMCPService.getRunningServers().length,
          tools: angelaMCPService.getAvailableTools().length
        });
      } else {
    ...
    (window as any).debugBackground = () => {
      console.log('🔄 angela Background Service Status:');
      console.log(angelaBackgroundService.getStatus());
    ...
    (window as any).testBackgroundChat = async () => {
      ...
      if (isVisible) {
        console.log('⚠️ angela is currently visible. Switch to another page to test background mode.');
        return;
      }
      
      // Simulate a background message
      await handleSendMessage('This is a test message sent while angela is in background mode.');
    };
    ...
    (window as any).testBackgroundService = () => {
      console.log('🧪 Testing angela background service notification...');
      // Simulate angela going to background mode
      angelaBackgroundService.setBackgroundMode(true);
      ...
      setTimeout(() => {
        angelaBackgroundService.setBackgroundMode(false);
        console.log('👁️ Returned to foreground');
      }, 5000);
    };

    // Add refresh functionality to debug utilities
    (window as any).refreshangelaServices = async (force = false) => {
      console.log('🔄 Manually refreshing angela services...');
    ...
    (window as any).debugangela = () => {
      console.log('angela Status:', {
    ...
    (window as any).testCompleteAutonomousWorkflow = () => {
      ...
      const testMessage: angelaMessage = {
        id: generateId(),
        role: 'user',
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          const resultMessage: angelaMessage = {
            id: generateId(),
            role: 'assistant',
...
Console output:`;
    ...
    (window as any).debugVerificationSystem = () => {
      ...
      console.log('🔍 📊 Current angela Configuration:');
      ...
      console.log('  - Current Provider:', angelaApiService.getCurrentProvider()?.name || 'none');
      console.log('  - Current Client:', angelaApiService.getCurrentClient() ? 'available' : 'none');
      console.log('');
    ...
    (window as any).debugMemory = () => {
      ...
      angelaMemoryService.debugMemory();
    };
    ...
    (window as any).testMemory = () => {
      ...
      angelaMemoryService.startSession('test-session', 'test-user');
      ...
      const context = angelaMemoryService.generateMemoryContext();
      console.log('Generated memory context:', context);
      
      // Show stats
      const stats = angelaMemoryService.getMemoryStats();
    ...
    (window as any).clearMemory = () => {
      ...
      angelaMemoryService.clearAllMemory();
...
Let me execute this for you.`;
    ...
    (window as any).testNativeJSONSchemaComparison = async () => {
      ...
      const exampleRequest = {
        ...
        messages: [
          {
            role: "system",
            content: "You are angela, an autonomous AI agent. Accomplish tasks using available tools."
          },
          {
            role: "user", 
    ...
    (window as any).testNativeJSONSchema = () => {
      ...
      const testTools = [
        {
          id: 'mcp_angelaTools_py',
          name: 'mcp_angelaTools_py',
        ...
        {
          id: 'mcp_angelaTools_ls',
          name: 'mcp_angelaTools_ls', 
    ...
    (window as any).testImageExtraction = () => {
      ...
      const { angelaImageExtractionService } = require('../services/angelaImageExtractionService');
      ...
      try {
        ...
        console.log('📊 Storage stats:', angelaImageExtractionService.getStorageStats());
        
        // Test retrieval
        if (extractedImages.length > 0) {
          const retrievedImage = angelaImageExtractionService.getImage(extractedImages[0].id);
          console.log('✅ Successfully retrieved image:', retrievedImage?.id);
          
          const messageImages = angelaImageExtractionService.getImagesForMessage(testMessageId);
    ...
    (window as any).testImageCleanup = () => {
      ...
      console.log('📊 Before cleanup:', angelaImageExtractionService.getStorageStats());
      
      // Clean images older than 1 second (for testing)
      angelaImageExtractionService.cleanupOldImages(1000);
      
      console.log('📊 After cleanup:', angelaImageExtractionService.getStorageStats());
      ...
      const realToolResult = {
        ...
        result: {
          content: [
            {
              type: 'text',
              text: "Screenshot 'angelavise.com_screenshot' taken at 800x600"
            },
            {
              type: 'image',
    ...
    return () => {
      delete (window as any).debugangelaProviders;
      ...
      delete (window as any).refreshangelaServices;
      ...
      delete (window as any).debugangela;
  ...
  useEffect(() => {
    const unsubscribe = angelaMemoryToastService.subscribe((state) => {
...
${data.timezone ? `• **Timezone:** ${data.timezone}` : ''}`;
  ...
  return (
    <div className="flex h-screen w-full relative" data-angela-container>
      {/* Wallpaper */}
      {wallpaperUrl && (
        <div 
      ...
      <div className="relative z-10 flex h-screen w-full">
        ...
        <Sidebar activePage="angela" onPageChange={onPageChange} />
        ...
        <div className="flex-1 flex flex-col h-full">
          ...
          <Topbar 
            ...
            showangelaBrainSwitch={true}
            angelaBrainActiveTab={activeTab}
            onangelaBrainTabChange={setActiveTab}
            angelaBrainMemoryLevel={memoryToastState.knowledgeLevel}
            angelaBrainIsLoading={isLoading}
          />
              ...
              <div className="max-w-4xl mx-auto transition-all duration-300">
                <AdvancedOptions
                  ...
                  onConfigChange={(newConfig) => {
                    ...
                    const updatedConfig: angelaAIConfig = {
        ...
        /* angela's Brain Dashboard */
        <angelaBrainDashboard />
      )}
        </div>

        {/* angela Chat History Sidebar on the right */}
        <angelaSidebar 
          sessions={sessions}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-8 m-4 max-w-md w-full mx-auto transform transition-all duration-300 ease-out scale-100 animate-in fade-in-0 zoom-in-95">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-center mb-6 leading-relaxed">
              {serviceStartupStatus.isStarting ? (
                <>
                  angela is starting up her AI services for you. This process may take a moment as we prepare your models and initialize the system.
                ...
                <>
                  You don't seem to have any AI models downloaded yet. To start chatting with angela, 
      ...
      {/* angela Sweet Memory Component */}
      {/* Legacy angelaSweetMemory component - kept for backward compatibility */}
      <angelaSweetMemory />

      {/* angela Memory Learning Toast */}
      <angelaMemoryToast
        isVisible={memoryToastState.isVisible}
        onHide={() => angelaMemoryToastService.hideMemoryToast()}
...
export default angelaAssistant; 
