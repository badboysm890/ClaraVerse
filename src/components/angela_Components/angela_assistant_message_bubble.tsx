/**
 * angela Assistant Message Bubble Component
 * 
 * This component renders individual chat messages in the angela assistant interface.
...
// Import TTS service
import { angelaTTSService, AudioControlState } from '../../services/angelaTTSService';
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[14px] font-medium ${getStatusColor()}`}>
            ...
            {getStatusText() === "thinking..." ? "angela is  " : "Thought for "} {getStatusText()} {"herself"}
            </span>
          </span>
        </div>
        <div className="flex items-center gap-2">
...
const FileAttachmentDisplay: React.FC<{ 
  attachment: angelaFileAttachment;
  onPreview?: (attachment: angelaFileAttachment) => void;
...
/**
 * Main angela Message Bubble Component
 */
const angelaMessageBubble: React.FC<angelaMessageBubbleProps> = ({
  message,
  userName = 'You',
  isEditable = false,
  onCopy,
  ...
  useEffect(() => {
    const unsubscribe = angelaTTSService.onHealthChange((isHealthy) => {
  ...
  useEffect(() => {
    const unsubscribe = angelaTTSService.onStateChange((state) => {
      setAudioState(state);
  ...
  const handleTTSToggle = async () => {
    ...
    try {
      ...
      await angelaTTSService.synthesizeAndPlayWithControls({
        text: cleanContent,
        engine: 'kokoro',
        voice: 'af_sarah',
        speed: 1.0,
  ...
  const handleFilePreview = (attachment: angelaFileAttachment) => {
...
export default angelaMessageBubble; 
