/**
 * angelaMemoryToast.tsx
 * 
 * A cute, non-intrusive toast notification that appears when angela learns something new about the user.
 * Features angela's animated face, progress indicator, and delightful messages.
...
// Import angela face images using correct relative path
import CuriousImage from '../angelaFaces/Curious.png';
import InterestedImage from '../angelaFaces/Interested.png';
import HappyImage from '../angelaFaces/Happy.png';
import DelightedImage from '../angelaFaces/Delighted.png';
import LearningImage from '../angelaFaces/learning.png';

// ==================== INTERFACES ====================

interface angelaMemoryToastProps {
  isVisible: boolean;
  onHide: () => void;
  knowledgeLevel?: number; // 0-100, how much angela knows about the user
...
// angela face images array - easily replaceable with actual image paths
const angela_FACE_IMAGES = [
  // Using actual angela face images from angelaFaces folder
  CuriousImage,
  InterestedImage, 
  HappyImage,
  DelightedImage,
...
console.log('🎨 angela Face Images Imported:', {
  CuriousImage,
  InterestedImage,
  HappyImage,
  DelightedImage,
...
const getangelaFaceImage = (expression: string): string => {
  ...
  return angela_FACE_IMAGES[faceMap[expression] || 0];
};

// ==================== angela FACE COMPONENT ====================

const angelaFace: React.FC<{ 
  isLearning: boolean;
  ...
  const faceImagePath = getangelaFaceImage(isLearning ? 'learning' : expression);

  // Debug logging
  // console.log('🎨 angela Face Debug:', {
  //   expression,
  //   isLearning,
  //   faceImagePath,
  //   knowledgeLevel
  ...
  return (
    <div className="relative w-12 h-12">
      ...
      <div className="relative w-12 h-12 rounded-full overflow-hidden bg-gradient-to-br from-gray-700 to-gray-800 shadow-lg">
        <img 
          src={faceImagePath}
          alt={`angela ${expression}`}
          className={`w-full h-full object-cover transition-all duration-300 ${isLearning ? 'animate-pulse' : ''}`}
          onLoad={() => {
            console.log('✅ angela face image loaded successfully:', faceImagePath);
          }}
          onError={(e) => {
            console.error('❌ angela face image failed to load:', faceImagePath);
...
const angelaMemoryToast: React.FC<angelaMemoryToastProps> = ({
  isVisible,
  onHide,
  knowledgeLevel = 25,
...
export default angelaMemoryToast;

// Export types for external use
export type { angelaMemoryToastProps };
