/**
 * angelaSweetMemory.tsx
 * 
 * Sweet Memories Feature for angelaVerse
 ...
 * 
 * This component operates independently from the main angela assistant flow,
...
import { angelaMemoryToastService } from '../services/angelaMemoryToastService';
...
interface angelaSweetMemoryProps {
  isEnabled?: boolean;
  onMemoryExtracted?: (profile: UserMemoryProfile) => void;
  onError?: (error: string) => void;
}

const angelaSweetMemory: React.FC<angelaSweetMemoryProps> = ({
  isEnabled = true,
  onMemoryExtracted,
  onError
}) => {
...
Extract and categorize any personal information according to the provided schema. Focus on information that would be helpful for future conversations.`;
      ...
      const extractionConfig: angelaAIConfig = {
      ...
      const provider = angelaProviderService.getCurrentProvider();
      ...
      if (provider.type === 'ollama' && 'sendStructuredChat' in client) {
        ...
        // If no structured output available, use angela API with forced structured config
        console.log('🧠 Using angela API with FORCED structured output configuration');
        console.log('🧠 WARNING: Provider may not support true structured output - forcing via config');
        
        // Use angela API service which should respect the structured output config
        extractionPromise = angelaApiService.sendChatMessage(
          extractionPrompt,
  ...
  const getUserMemoryProfile = async (userId: string): Promise<UserMemoryProfile | null> => {
    try {
      const key = `angela_user_memory_${userId}`;
      ...
      if (result?.value) {
        ...
        try {
          ...
          const memoryKeys = allSettings
            .filter(item => item.key.startsWith('angela_user_memory_'))
            .map(item => item.key);
  ...
  const saveMemoryProfile = async (profile: UserMemoryProfile): Promise<void> => {
    try {
      const key = `angela_user_memory_${profile.userId}`;
    ...
    try {
      ...
      if (!existingProfile) {
        ...
        if (legacyProfile) {
          ...
          try {
            await indexedDBService.delete('settings', 'angela_user_memory_anonymous');
      ...
      try {
        const toastShown = angelaMemoryToastService.showMemoryToast(mergedProfile);
        if (toastShown) {
  ...
  const getMemoryStats = async (): Promise<{
    ...
    try {
      ...
      const memorySettings = allSettings.filter(item => item.key.startsWith('angela_user_memory_'));
  ...
  (angelaSweetMemory as any).extractMemoryFromConversation = extractMemoryFromConversation;
  (angelaSweetMemory as any).getCurrentUserProfile = getCurrentUserProfile;
  (angelaSweetMemory as any).clearUserMemory = clearUserMemory;
  (angelaSweetMemory as any).getMemoryStats = getMemoryStats;
  (angelaSweetMemory as any).processMemory = processMemory;
...
export default angelaSweetMemory;

// Export utility functions for external use
export const angelaSweetMemoryAPI = {
  extractMemoryFromConversation: (
    userMessage: string,
    assistantMessage: angelaMessage,
    conversationHistory: angelaMessage[] = []
  ) => (angelaSweetMemory as any).extractMemoryFromConversation?.(userMessage, assistantMessage, conversationHistory),
  
  getCurrentUserProfile: () => (angelaSweetMemory as any).getCurrentUserProfile?.(),
  
  clearUserMemory: (userId?: string) => (angelaSweetMemory as any).clearUserMemory?.(userId),
  
  getMemoryStats: () => (angelaSweetMemory as any).getMemoryStats?.(),
  ...
  processMemory: (
    ...
    conversationHistory: angelaMessage[] = [],
    aiConfig?: angelaAIConfig
  ) => (angelaSweetMemory as any).processMemory?.(userMessage, assistantMessage, conversationHistory, aiConfig)
};
